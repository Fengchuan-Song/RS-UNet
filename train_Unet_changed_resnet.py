...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    # model_path = "logs/loss_2022_08_15_08_50_11/ep139-loss0.010-val_loss0.009.pth"
    model_path = ""
    # ------------------------------#
    #   输入图片的大小
    # ------------------------------#
    ...
    model = Net(num_classes=num_classes, r=[8, 8, 8, 8], pretrained=pretrained).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        # ------------------------------------------------------#
        ...
        #     # try except语句块用于针对不同Decoder加载同一Encoder权重
        #     try:
        #         model_dict[k]
        #     except KeyError:
        #         break
    ...
    if True:
        ...
        for epoch in range(start_epoch, end_epoch):
            best_dice = fit_one_epoch(model_train, model, loss_history, optimizer, epoch,
                          epoch_step, epoch_step_val, gen, gen_val, end_epoch, Cuda, dice_loss, focal_loss, cls_weights,
                          num_classes, best_dice, save_path)
            lr_scheduler.step()
