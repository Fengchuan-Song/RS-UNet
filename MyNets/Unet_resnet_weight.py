...
class Distillation(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv_1 = nn.Conv2d(in_channels=in_channels, out_channels=in_channels, kernel_size=(1, 1))
        self.conv_2 = nn.Conv2d(in_channels=in_channels, out_channels=in_channels, kernel_size=(1, 1))

    def forward(self, input_lower, input_higher):
        input_lower = self.conv_1(input_lower)
        input_higher = self.conv_2(input_higher)

        return input_lower + input_higher

class Distillation_1(nn.Module):
    def __init__(self, in_channels):
        super(Distillation_1, self).__init__()
        # self.weight_input = nn.Parameter(torch.Tensor(1, in_channels, 1, 1), requires_grad=True)
        # self.weight_skip = nn.Parameter(torch.Tensor(1, in_channels, 1, 1), requires_grad=True)
        
        self.weight_input = nn.Parameter(torch.Tensor(1, 1), requires_grad=True)
        self.weight_skip = nn.Parameter(torch.Tensor(1, 1), requires_grad=True)

    def forward(self, input_lower, input_higher):
        input_lower = self.weight_input * input_lower
        input_higher = self.weight_skip * input_higher
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, reduction):
        ...
        self.conv3x3 = nn.Conv2d(in_channels=in_channels[0], out_channels=in_channels[0], kernel_size=(3, 3),
                                 stride=(1, 1), padding=1)
        self.up_sample = nn.ConvTranspose2d(in_channels=in_channels[1], out_channels=in_channels[0],
                                            kernel_size=(2, 2), stride=(2, 2))
        self.distillation = Distillation(in_channels=in_channels[0])

        self.attention = SEBlock(num_channels=in_channels[0], rate=reduction)
    ...
    def forward(self, input_1, input_2):
        ...
        # output = self.distillation(input_1, input_2)
