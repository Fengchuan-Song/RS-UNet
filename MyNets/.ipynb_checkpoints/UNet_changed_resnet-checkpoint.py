...
from MyNets.Encoder_resnet import Encoder
# from MyNets.Net_CA import Encoder
...
class UpBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        # self.conv1x1 = nn.Conv2d(in_channels=(2 * in_channels[0]), out_channels=in_channels[0], kernel_size=(1, 1),
        #                          stride=(1, 1))
...
class Net(nn.Module):
    ...
    def __init__(self, num_classes, r, pretrained=False):
        ...
        self.encoder = Encoder(in_channels=3, num_channels=64, r=r, pretrained=pretrained)

        self.up_block4 = UpBlock(in_channels=[256, 512])
        self.up_block3 = UpBlock(in_channels=[128, 256])
        self.up_block2 = UpBlock(in_channels=[64, 128])
        self.up_block1 = UpBlock(in_channels=[32, 64])

        self.final = nn.Conv2d(in_channels=64, out_channels=num_classes, kernel_size=(1, 1),
                               stride=(1, 1), padding=0)
        # self.final = nn.ConvTranspose2d(in_channels=32, out_channels=num_classes, kernel_size=(2, 2),
        #                                 stride=(2, 2))
    ...
    def freeze_backbone(self):
        for params in self.encoder.backbone.conv1.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.bn1.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.relu.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.maxpool.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.layer1.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.layer2.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.layer3.parameters():
            params.requires_grad = False
        for params in self.encoder.backbone.layer4.parameters():
            params.requires_grad = False
